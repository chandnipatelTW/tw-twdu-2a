...
nohup spark-submit --master yarn --queue streaming --deploy-mode cluster --class com.free2wheelers.apps.StationApp --name StationApp --packages org.apache.spark:spark-sql-kafka-0-10_2.11:2.3.0  --driver-memory 1000M --conf spark.executor.memory=2g --conf spark.cores.max=1 --driver-java-options "-Dlog4j.configuration=file:/tmp/log4j.properties" \
